      ...
      <div className="login h-full w-full flex justify-center items-center mt-5 text-center">
        <form className=" h-[450px] w-[330px] rounded-lg border-2 flex flex-col justify-around items-center" onSubmit={loginHandler}>
          ...
          <div className="flex flex-col gap-10">
            <input type='email' required placeholder='Enter Your username' className="w-4/5 rounded-md mx-auto" value={email} onChange={(e) => { setEmail(e.target.value) }} />
            <input type='password' required placeholder='Enter Your password' className="w-4/5 rounded-md mx-auto" value={password} onChange={(e) => { setPassword(e.target.value) }} />
          </div>
          <div>
